# http://old.peihua.cn/stuyqcj/index.aspx?name=34B3180223

import requests as rq
from bs4 import BeautifulSoup


def postHealthReport(name, zhenshu):
    url = 'http://old.peihua.cn/stuyqcj/index.aspx?name=' + name  # 需要请求的URL地址
    data1 = {'__EVENTTARGET': '',
             '__EVENTARGUMENT': '',
             '__VIEWSTATE': '',
             '__EVENTVALIDATION': '',
             'ssq': '陕西省西安市长安区',
             '1': 'A2',
             '2': 'B1',
             '3': 'C1',
             '4': 'D2',
             '8': 'E3',
             'F1': 'on',
             'F2': 'on',
             'F3': 'on',
             'F4': 'on',
             'lynr1': '无',
             'Button1': '提    交'
             }  # POST请求需要提交的数据

    data2 = {'__EVENTTARGET': '',
             '__EVENTARGUMENT': '',
             '__VIEWSTATE': '',
             '__EVENTVALIDATION': '',
             'ssq': '陕西省西安市长安区',
             '1': 'A2',
             '2': 'B2',
             '3': 'C1',
             '4': 'D2',
             '8': 'E3',
             'F1': 'on',
             'F2': 'on',
             'F3': 'on',
             'F4': 'on',
             'lynr1': '无',
             'Button1': '提    交'
             }  # POST请求需要提交的数据

    headers = {'content-type': 'application/x-www-form-urlencoded'}
    if zhenshu == 2:
        rq.post(url=url, data=data2, headers=headers)
    else:
        rq.post(url=url, data=data1, headers=headers)
    res_second = rq.post(url=url, headers=headers)
    return name + BeautifulSoup(res_second.text, 'html.parser').find("h2").getText()

if __name__ == '__main__':
    print(postHealthReport('34B3190102', 1))

    print(postHealthReport('34B3190108', 1))
    print(postHealthReport('34B3190120', 1))
    print(postHealthReport('34B3180223', 2))
    print(postHealthReport('34B3190131', 1))
    print(postHealthReport('34B3190136', 1))
